...
$("document").ready(()=>{
    ...
    searchAccountNumber();
    searchName();
    displayPayeeDetails(payeeStore);
    updatePagination();
...
const filteredPayee=payeeArray.filter(payee=>payee.accountNumber === accountNumberInput);
...
const accountNumberInput=$("#accountNumberInput").val();
const filteredPayee=payeeArray.filter(payee=>payee.payeeName === nameInput && payee.accountNumber === accountNumberInput);
if(filteredPayee.length>0){
   displayPayeeDetails(filteredPayee)
}else{
    alert("Payee not found")
