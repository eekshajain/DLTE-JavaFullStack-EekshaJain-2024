...
public class UserDatabaseRepository implements UserRepository {
        ...
        try {
            ...
            if (resultSet.next()) {
                if (!(resultSet.getString(1).equals(username) && resultSet.getString(2).equals(password))) {
                    throw new WithdrawException(); // Throwing exception when verification fails
                }
                return true;
            } else {
                throw new WithdrawException(); // Throwing exception when user not found
            }
        }  catch (SQLException e) {
            e.printStackTrace();
            ...
            for (int attempts = 2; attempts <= 3; ) {
                ...
                System.out.println(resourceBundle.getString("accounts.wrong.password"));
    ...
    public void addAccounts(Account account) {
        try {
            ...
            if (result != 0) {
                logger.log(Level.INFO, resourceBundle.getString("card.ok"));
                System.out.println(resourceBundle.getString("card.ok"));
            } else {
                logger.log(Level.INFO, resourceBundle.getString("card.fail"));
                System.out.println(resourceBundle.getString("card.fail"));
            }
        } catch (SQLException sqlException) {
